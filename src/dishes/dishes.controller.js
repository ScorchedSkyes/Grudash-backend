...
function create(req, res) {
   const newDishData = res.locals.newDishData;
...
function update(req, res) {
   const newData = res.locals.newDishData;
...
const dishHasValidDescription = (req, res, next) => {
   const dishDescription = res.locals.newDishData.description;
...
const dishHasValidPrice = (req, res, next) => {
   const dishPrice = res.locals.newDishData.price;
...
const dishHasValidImage = (req, res, next) => {
   const dishImage = res.locals.newDishData.image_url;
...
const dishIdMatches = (req, res, next) => {
   ...
   const { id } = res.locals.newDishData;
   if (paramId != id && id) {
      return next({
         status: 400,
