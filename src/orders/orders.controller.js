...
const existingStatusIsValid = (req, res, next) => {
...
const existingStatusIsPending = (req, res, next) => {
   const { status } = res.locals.order;
   if (status !== "pending") {
      return next({
         status: 400,
...
const updateValidation = (req, res, next) => {
   ...
   existingStatusIsValid(req, res, next);
   next();
};

const deleteValidation = (req, res, next) => {
   orderExists(req, res, next);
   existingStatusIsPending(req, res, next);
   next();
};
